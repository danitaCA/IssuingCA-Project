...
public class SerializationService {
    ...
    /**
     * Serializes an X509CRLHolder into binary DER format.
     */
    public byte[] convertToDer(org.bouncycastle.cert.X509CRLHolder crlHolder) throws Exception {
        return crlHolder.getEncoded();
    }

    /**
     * Parses an X509CRLHolder from a PEM string.
     */
    public org.bouncycastle.cert.X509CRLHolder parseCrlFromPem(String pemContent) throws Exception {
        try (PEMParser parser = new PEMParser(new StringReader(pemContent))) {
            Object obj = parser.readObject();
            if (obj instanceof org.bouncycastle.cert.X509CRLHolder) {
                return (org.bouncycastle.cert.X509CRLHolder) obj;
            }
            throw new IOException("No valid CRL found in stream");
        }
    }

    /**
     * Parses an X509CRLHolder from binary DER format.
     */
    public org.bouncycastle.cert.X509CRLHolder parseCrlFromDer(byte[] derBytes) throws Exception {
        return new org.bouncycastle.cert.X509CRLHolder(derBytes);
    }
}
